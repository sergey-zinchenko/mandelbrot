...
void inline __attribute__((always_inline)) Mandelbrot_0_simd(int h, uint *pResult) {
    ...
    for (int w = 0; w < WIDTH; w += 16) {
        ...
        __m256 cxVec1 = _mm256_mul_ps(DisplacementVector, ScaleXVec);
        __m256 cxVec2 = _mm256_mul_ps(DisplacementVector, ScaleXVec);
        __m256 cyVec = _mm256_mul_ps(cyVecset, ScaleYVec);
        cxVec1 = _mm256_add_ps(cxVec1set, cxVec1);
        cxVec2 = _mm256_add_ps(cxVec2set, cxVec2);
        cyVec = _mm256_add_ps(MinYVec, cyVec);
        __m256 mag2Vec1;
        __m256 mag2Vec2;
        int i1 = 0;
        int i2 = 0;
        ...
        do {
            ...
            zReVec1 = _mm256_add_ps(
                    _mm256_sub_ps(_mm256_mul_ps(zReNewVec1, zReNewVec1), _mm256_mul_ps(zImNewVec1, zImNewVec1)),
                    cxVec1);
            zReVec2 = _mm256_add_ps(
                    _mm256_sub_ps(_mm256_mul_ps(zReNewVec2, zReNewVec2), _mm256_mul_ps(zImNewVec2, zImNewVec2)),
                    cxVec2);
